...
--
-- Dependencies: right_shifter.vhd
...
architecture Behavioral of FPP_ADD_SUB is
    constant K: natural:= 32;
    constant P: natural:= 24;
    constant E: natural:= 8;
    constant add_sub : std_logic := '1';
    ...
    signal go_del : std_logic := '0';
    signal done_i : std_logic := '0';
begin

    -- for timing (give the logic some time to do its thing)
    ...
    sign_A <= A_int(K-1); --in single precision 31
    exp_A <= A_int(K-2 downto K-E-1); --in single(30 .. 23);
    
    sign_B <= B_int(K-1) when add_sub='1' else not B_int(K-1); --effective sign of operator_B
    exp_B <= B_int(K-2 downto K-E-1); --in single(30 .. 23);
